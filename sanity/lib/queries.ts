...
export async function getSource() {
  ...
  const source = await client.fetch(`*[_type == "source"][0] { body[] { ..., _type == "image" => { ..., "url": asset->url } } }`, {}, { next: { revalidate: 3600 } })
  if (source?.body) return source
  const teaching = await client.fetch(`*[_type == "teaching"][0] { title, description[] { ..., _type == "image" => { ..., "url": asset->url } } }`, {}, { next: { revalidate: 3600 } })
